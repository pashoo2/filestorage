  ...
  implements IFileStorage<T>
{
  ...
  protected getServiceByFileAddr(
    addr: TFileStorageFileAddress
  ): IFileStorageService<T> | undefined {
  ...
  protected connectToService = async (
    ...
    const ServiceConstructor = await this.getServiceConstructorByType(
      configuration.type
    );
    const service = new (ServiceConstructor.default ?? ServiceConstructor)();
